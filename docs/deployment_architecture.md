...
La arquitectura de esta fase del proyecto se basa en la continuación de la anterior arquitectura avanzando a un entorno que permita la escalabilidad horizontal. Este es el diseño de la arquitetcura final:


- **🗄️ Amazon Relational Database Service (RDS):** Ejecuta la base de datos MySQL de la aplicación. Se comunica mediante la Virtual Private Cloud (VPC) de Amazon unicamente con las tareas de Amazon Elastic Container Service (ECS). Gracias a los Amazon Security Goups esta base de datos solo acepta trafico proveniente del puerto 3306 desde la ECS, sin exponer ningun puerto más.

- **💾 Amazon Simple Storage Service (S3):** Proporciona almacenamiento de objetos para la gestión de archivos e imágenes. Este servicio se encuentra fuera de la VPC por lo que tenemos que usar Amazon Identity and Access Management (IAM) para lograr comunicar el servicio ECS y sus tareas con el servicio S3 mediante roles de identidad.

- **Amazon Virtual Private Cloud (VPC):** Se divide en dos zonas de disponibilidad para garantizar la mayor disponibilidad posible del servicio y estas se dividen en dos subredes las cuales poseen en su interior replicas de la aplicación. Esta división permite que aun si falalra un centro de datos entero de Amazon la aplicación seguiria funcionando.
