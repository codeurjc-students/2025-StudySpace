...
export class ManageReservationsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.roomsService
      .getRooms(0, 100)
      .subscribe((data) => (this.rooms = data.content));
  }
  ...
  startEdit(reservation: any) {
    ...
    this.editStartTime =
      ReservationLogic.pad(startDate.getHours()) +
      ':' +
      ReservationLogic.pad(startDate.getMinutes());
    this.editEndTime =
      ReservationLogic.pad(endDate.getHours()) +
      ':' +
      ReservationLogic.pad(endDate.getMinutes());
  ...
  onConfigChange(isInit: boolean = false) {
    ...
    this.reservationService
      ...
      .subscribe((data) => {
        this.occupiedSlots = data.filter(
          (r: any) => r.id !== this.editingReservation.id,
        );

        this.availableStartTimes = ReservationLogic.generateStartTimes(
          this.occupiedSlots,
        );
  ...
  onStartTimeChange(preselectedEndTime: string | null = null) {
    ...
    this.availableEndTimes = ReservationLogic.generateEndTimes(
      this.editStartTime,
      this.occupiedSlots,
    );
